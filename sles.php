...
<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="myModal23" class="modal fade">
									...
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Book:</label>
										...
										<div class="col-sm-3">
											...
											include('connect.php');
												...
												$result = $db->prepare("SELECT * FROM book");
												$result->bindParam(':userid', $res);
												$result->execute();
												for($i=0; $row11 = $result->fetch(); $i++){
												?>
												
												
												<option value="<?php echo $row11['book_name'];?>"><?php echo $row11['book_name'];?></option>
												<?php
												}
												?>
												</select>
									...
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Page No: </label>

										<div class="col-sm-9">
											<input type="text" id="form-field-1-1" name="pageNo" placeholder="" class="form-control" required/>
										</div>
		...
		<div class="tab-content" style=" width:100%; height: 400px; background-color:white; float:left; overflow-y:scroll;}">
										...
										<select id="lunch" name="users" class="selectpicker" data-live-search="true" onchange="showUser(this.value)" title="Please select a  item">
										<?php
										...
										for($i=0; $row = $result->fetch(); $i++){
											...
											<div style="height:50px;color:black;">
												<?php
												$pcat=$row['CaId'];
												$sql="SELECT * FROM product_cat WHERE CaId=$pcat ";
												$result44=$db->prepare($sql);
												$result44->execute();
												for($i=0; $row44 = $result44->fetch(); $i++){
													?>
													<p style="color: white;"><u><?php echo  $row44['Cat_Name'];?></u></p>
													<?php
												}
				...
				<input type="hidden" name="kk" value="0" id="my-input" >
                    <div class="col-md-12 product_img">
                        ...
                        <div class="btn-ground">
                            <button type="submit" class="btn btn-primary"><span class="glyphicon glyphicon-shopping-cart"></span> Add</button>
